...
void PID_init_extern(pid_type_def *pid, uint8_t mode, const fp32 PID[3], fp32 max_out, fp32 max_iout, fp32 min_out, fp32 min_iout)
{
    if (pid == NULL || PID == NULL)
    {
        return;
...
fp32 PID_Calc(pid_type_def *pid, fp32 ref, fp32 vref,fp32 set)
{
    if (pid == NULL)
    {
        return 0.0f;
